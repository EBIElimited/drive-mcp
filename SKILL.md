...
2. Properties: `list_units` → `get_unit` → `update_unit` for empty fields (squareMeters, rooms). Always pass `versionReason`. Bad write: `list_unit_versions` then `restore_unit`. Trail: `list_unit_documents` → `download_unit_document`.
