...
5. Mail: `list_mail_accounts` → `search_mail` → `read_mail`. Reply with `create_mail_draft` (lands in Drafts; the user sends it — agents cannot send). Triage with `manage_mail` (read/unread, flag, Trash, restore). No passwords.
