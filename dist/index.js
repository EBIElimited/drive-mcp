 ...
 */
function contentBytesToMcp(c, filename, fileId) {
    if (TEXT_MIME_PATTERN.test(c.mimeType)) {
        ...
        return [
            { type: 'text', text: c.partial ? `(partial, ${c.size} bytes)\n${text}` : text },
        ];
    }
    if (c.mimeType.startsWith('image/')) {
        return [
            { type: 'image', data: bytesToBase64(c.bytes), mimeType: c.mimeType },
        ];
    }
...
// ── Identity / Discovery ────────────────────────────────────────────────────
server.tool('whoami', 'Show the authenticated Achi Drive user, auth method, and whether the token has file-content access.', {}, async () => {
    try {
...
server.tool('list_teams', "List all teams the user is a member of, with their roles. Use the returned team IDs as the `teamId` arg to other tools to operate inside a team's space.", {}, async () => {
    try {
...
server.tool('get_file', 'Get metadata for a single file (name, mimeType, sizeBytes, etc.). Does NOT return content — use read_file for that.', { id: z.string().uuid() }, async ({ id }) => {
    try {
...
server.tool('get_folder', 'Get metadata for a folder.', { id: z.string().uuid() }, async ({ id }) => {
    try {
...
server.tool('search', 'Search files and folders by name (case-insensitive substring match). Scans up to 5,000 items per scope; results may be truncated for huge drives.', {
...
server.tool('read_file_text', 'Read a file as plain text, decoded as UTF-8. Convenience wrapper around read_file with a default 1 MB cap. Returns an error if the file is not text-encodable.', {
    id: z.string().uuid(),
...
server.tool('read_thumbnail', 'Get the JPEG thumbnail of a file (if it has one). Returns MCP image content. Useful for previewing images/videos without downloading the full file.', { id: z.string().uuid() }, async ({ id }) => {
    try {
        const content = await client.readThumbnail(id);
        return {
            content: [{ type: 'image', data: bytesToBase64(content.bytes), mimeType: content.mimeType }],
        };
    }
    catch (err) {
        return errorResult(err);
...
// ── Mutations ───────────────────────────────────────────────────────────────
server.tool('upload_file_from_path', 'THE large-file uploader. Pass a local disk path (zips, videos, anything). Streams 5 MiB plaintext chunks. Use this for anything over a few MB — never base64 a zip.', {
...
server.tool('upload_file', `SMALL FILES ONLY (under ${INLINE_UPLOAD_MAX_BYTES} bytes). Text or base64 notes. Refuses larger blobs — use upload_file_from_path. Never base64 a zip.`, {
...
server.tool('update_file', 'Rename, move, star/unstar, trash/restore a file in one call. Pass only the fields you want to change.', {
    id: z.string().uuid(),
...
server.tool('delete_file', 'Move a file to trash, or delete it permanently (irreversible). Default is trash.', {
    id: z.string().uuid(),
...
server.tool('update_folder', 'Rename, move, star/unstar, trash/restore a folder.', {
    id: z.string().uuid(),
    name: z.string().min(1).max(512).optional(),
    starred: z.boolean().optional(),
...
server.tool('delete_folder', 'Move a folder to trash (recursive — all descendants trashed) or delete it permanently (also recursive — all contents wiped from R2 + DB). Permanent delete is irreversible.', {
    id: z.string().uuid(),
...
server.tool('list_unit_versions', 'List Properties unit snapshots (newest first). Use restore_unit if an earlier write was wrong.', {
    unitId: z.string().uuid(),
...
server.tool('restore_unit', 'Revert a Properties unit to a prior snapshot. The live row is snapshotted first so this restore can also be undone.', {
    unitId: z.string().uuid(),
...
server.tool('list_unit_documents', 'List the Properties document trail for an apartment (HV, heating, tax, prior NK letters).', { unitId: z.string().uuid() }, async ({ unitId }) => {
    try {
...
server.tool('list_unit_payments', 'Bank-statement payment trail for an apartment (cold/warm/NK). GET only — do not PATCH. Amount corrections belong in Achi Zahlungen / the matching job.', {
    unitId: z.string().uuid(),
...
server.tool('get_landlord_profile', 'Stored Vermieter letterhead for a space. Empty fields stay empty — never invent legal name, street, or IBAN.', { teamId: z.string().optional() }, async (args) => {
    try {
...
server.tool('list_bank_transactions', 'Kontoauszug lines for a team space. Requires teamId.', {
    teamId: z.string(),
...
server.tool('list_mail_accounts', 'Mailboxes the user can read. Never returns passwords.', { teamId: z.string().optional() }, async (args) => {
    try {
...
server.tool('manage_mail', 'Triage mail in Achi: mark read/unread, flag/unflag, move to Trash or back to Inbox. Up to 100 message ids per call. Needs manage access to the mailbox. Changes Achi, not the mail server.', {
...
server.tool('create_property_visit', 'Log a viewing trip. distanceKm is one-way. roundTrip defaults true (Hin- und Rückfahrt, deductible ×2). Never invent kilometres. Default rate 0.30 €/km.', {
    teamId: z.string().optional(),
...
server.tool('update_property_visit', 'Update a viewing trip (status, distanceKm, roundTrip, notes). Never invent kilometres.', {
    id: z.string().uuid(),
...
server.tool('get_financials_book', 'Open the space\'s USD books: chart of accounts, bank pots (ids for import_bank_csv), coding rules, flags. Never invent FX rates.', { teamId: z.string().optional() }, async (args) => {
    try {
...
server.tool('import_bank_csv', 'Import a Mercury or Wise CSV export as-is. Idempotent. dryRun previews. A Wise rate counts only for conversions to/from USD; other foreign lines are flagged missing_usd_rate with USD left null (use set_transaction_rate).', {
...
server.tool('categorize_transaction', 'Post a balanced USD journal for a bank line, once (409 ALREADY_POSTED on retry). Fails with missing_rate if a foreign line has no rate. dryRun previews.', {
    id: z.string().uuid(),
    teamId: z.string().optional(),
...
server.tool('exclude_transaction', 'Exclude an open line that belongs to another entity (e.g. Chi Ross / DE rentals in the Elania books).', { id: z.string().uuid(), teamId: z.string().optional() }, async ({ id, teamId }) => {
    try {
...
server.tool('set_transaction_rate', 'Record a USD rate (USD per 1 unit of the line currency) on an open foreign line, e.g. from the Wise app or invoice. Stored as a manual rate and audited. Only from a real source — never guess.', { id: z.string().uuid(), rate: z.string().describe('e.g. "1.0850"'), teamId: z.string().optional() }, async ({ id, rate, teamId }) => {
    try {
...
server.tool('attach_receipt', 'Attach an uploaded receipt (documentId from upload_receipt or list_receipts) to a bank line. Idempotent; clears missing_invoice_pdf.', { transactionId: z.string().uuid(), documentId: z.string().uuid(), teamId: z.string().optional() }, async ({ transactionId, documentId, teamId }) => {
    try {
...
server.tool('detach_receipt', 'Remove a receipt from a bank line (the file stays in the books).', { transactionId: z.string().uuid(), documentId: z.string().uuid(), teamId: z.string().optional() }, async ({ transactionId, documentId, teamId }) => {
    try {
...
server.tool('get_report_pnl', 'P&L from posted journals only. Never invent.', { teamId: z.string().optional(), from: z.string().optional(), to: z.string().optional(), department: z.string().optional().describe('Department code, or none for company-level lines') }, async (args) => {
    try {
...
server.tool('list_departments', 'Departments (classes) inside the company\'s books, e.g. EGS Elania Game Studio, NH NeonHappi. Untagged = the company itself.', { teamId: z.string().optional() }, async (args) => {
    try {
...
server.tool('create_department', 'Add a department to the company\'s books.', { code: z.string(), name: z.string(), teamId: z.string().optional() }, async (args) => {
    try {
...
server.tool('set_transaction_department', 'Tag a bank line with a department (null untags); a posted line\'s journal follows. Only tag when the source says which product.', { id: z.string().uuid(), departmentCode: z.string().nullable(), teamId: z.string().optional() }, async (args) => {
    try {
...
server.tool('get_pnl_by_department', 'P&L per department plus company-level lines; columns add up to the company P&L.', { from: z.string().optional(), to: z.string().optional(), teamId: z.string().optional() }, async (args) => {
    try {
...
server.tool('get_report_cash', 'Cash per bank pot in native currency. USD (amountHome) only when every line has a rate; otherwise null with missingRateCount.', { teamId: z.string().optional(), asOf: z.string().optional() }, async (args) => {
    try {
